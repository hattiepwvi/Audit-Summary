...
/**
 * @title 总结：管理漏洞赏金计划的注册和参数配置。
 *        1）创建和管理 Vaults(每个 Vault 都是独立的漏洞赏金计划) => 设置漏洞赏金的参数（提现、安全期限、手续费、可见性）=> 奖励分配 （vesting 合约）=> 兑换 Token(将指定资产兑换成 HAT 代币、将特定资产的奖励添加到兑换池)
 *        2）设置和修改 Vault 的实现合约、设置默认仲裁者、暂停合约功能
 * @author
 * @notice
 */
...
contract HATVaultsRegistry is IHATVaultsRegistry, Ownable {
    ...
    function logClaim(string calldata _descriptionHash) external payable {
        ...
        if (_claimFee > 0) {
            if (msg.value < _claimFee) revert NotEnoughFeePaid();
    ...
    function setDefaultChallengePeriod(
        uint32 _defaultChallengePeriod
    ) external onlyOwner {
    ...
    function setWithdrawRequestParams(
        ...
        emit SetWithdrawRequestParams(
            _withdrawRequestPendingPeriod,
            _withdrawRequestEnablePeriod
        );
    }
    ...
    function createVault(
        ...
        IHATClaimsManager(vaultClaimsManager).initialize(
            IHATVault(vault),
            _claimsManagerParams
        );
    ...
    function addTokensToSwap(
        ...
        _asset.safeTransferFrom(
            msg.sender,
            address(this),
            _hackersHatReward + _governanceHatReward
        );
    }
    ...
    function swapAndSend(
        ...
        for (uint256 i = 0; i < _beneficiaries.length; ) {
            _swapData.hackerRewards[i] = hackersHatReward[_asset][
                _beneficiaries[i]
            ];
        ...
        IERC20 _HAT = HAT;
        (_swapData.hatsReceived, _swapData.amountUnused) = _swapTokenForHAT(
            IERC20(_asset),
            _swapData.amount,
            _amountOutMinimum,
            _routingContract,
        ...
        _swapData.governanceAmountSwapped = _swapData.usedPart.mulDiv(
            _swapData.governanceHatReward,
            _swapData.amount
        );
        governanceHatReward[_asset] = _swapData.amountUnused.mulDiv(
            _swapData.governanceHatReward,
            _swapData.amount
        );

        for (uint256 i = 0; i < _beneficiaries.length; ) {
            uint256 _hackerReward = _swapData.hatsReceived.mulDiv(
                _swapData.hackerRewards[i],
                _swapData.amount
            );
            uint256 _hackerAmountSwapped = _swapData.usedPart.mulDiv(
                _swapData.hackerRewards[i],
                _swapData.amount
            );
            _swapData.totalHackerReward += _hackerReward;
            hackersHatReward[_asset][_beneficiaries[i]] = _swapData
                .amountUnused
                .mulDiv(_swapData.hackerRewards[i], _swapData.amount);
            address _tokenLock;
            if (_hackerReward > 0) {
        ...
        uint256 _amountToOwner = _swapData.hatsReceived -
            _swapData.totalHackerReward;
        _HAT.safeTransfer(_owner, _amountToOwner);
        emit SwapAndSend(
            _owner,
            _swapData.governanceAmountSwapped,
            _amountToOwner,
            address(0)
        );
    }
    ...
    function owner()
        ...
        view
        virtual
        override(IHATVaultsRegistry, Ownable)
        returns (address)
    {
    ...
    function validateChallengeTimeOutPeriod(
        uint32 _challengeTimeOutPeriod
    ) public pure {
        if (_challengeTimeOutPeriod < 2 days)
            revert ChallengeTimeOutPeriodTooShort();
        if (_challengeTimeOutPeriod > 125 days)
            revert ChallengeTimeOutPeriodTooLong();
    }
    ...
    function _swapTokenForHAT(
        ...
        address _routingContract,
        bytes calldata _routingPayload
    ) internal returns (uint256 hatsReceived, uint256 amountUnused) {
        IERC20 _HAT = HAT;
        if (_asset == _HAT) {
            return (_amount, 0);
        }
        ...
        amountUnused =
            _amount -
            (_assetBalanceBefore - _asset.balanceOf(address(this)));
