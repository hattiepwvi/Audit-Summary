...
/**
 * @title 总结： 管理奖励池（管理并分配奖励）
 * @author 1）奖励控制器初始化（奖励代币的地址、管理员地址、奖励开始块、奖励周期长度等）=> 设置奖励分配点数（分配比例）=> 更新奖励池（奖励每份份额）=> 周期奖励（设置每个奖励周期内的奖励代币数量）
 *         2）用户奖励池变化（更新奖励债务和未领取奖励）=> 领取奖励 => 查询奖励 => 提取代币
 * @notice
 */
...
contract RewardController is IRewardController, OwnableUpgradeable {
    ...
    function setAllocPoint(
        ...
        if (_globalVaultsUpdatesLength != 0) {
            ...
            unchecked {
                // only used in case _globalVaultsUpdatesLength > 0
                _globalVaultsUpdatesLastIndex = _globalVaultsUpdatesLength - 1;
            }
            VaultUpdate storage vaultUpdate = globalVaultsUpdates[
                _globalVaultsUpdatesLastIndex
            ];
            _totalAllocPoint =
                vaultUpdate.totalAllocPoint -
                vaultInfo[_vault].allocPoint +
                _allocPoint;
    ...
    function updateVault(address _vault) public {
        ...
        if (_lastRewardBlock == 0) {
            ...
            vault.lastRewardBlock = block.number > _startBlock
                ? block.number
                : _startBlock;
            return;
        }
        ...
        if (_totalShares != 0) {
            ...
            vault.rewardPerShare += _reward.mulDiv(
                REWARD_PRECISION,
                _totalShares
            );
        }
        ...
        emit VaultUpdated(
            ...
            vault.lastProcessedVaultUpdate
        );
    }

    /** @notice See {IRewardController-setEpochRewardPerBlock}. */
    function setEpochRewardPerBlock(
        uint256[24] calldata _epochRewardPerBlock
    ) external onlyOwner {
        ...
        if (block.number < _startBlock) {
            ...
            for (; _nextEpoch < NUMBER_OF_EPOCHS; ++_nextEpoch) {
                epochRewardPerBlock[_nextEpoch] = _epochRewardPerBlock[
                    _nextEpoch
                ];
            }
    ...
    function _commitUserBalance(
        ...
        mapping(address => uint256) storage vaultRewardDebt = rewardDebt[
            _vault
        ];
        if (_userShares != 0) {
            unclaimedReward[_vault][_user] +=
                _userShares.mulDiv(_rewardPerShare, REWARD_PRECISION) -
                vaultRewardDebt[_user];
        }
        ...
        uint256 _newRewardDebt = _userShares.mulDiv(
            _rewardPerShare,
            REWARD_PRECISION
        );
        vaultRewardDebt[_user] = _newRewardDebt;
        emit UserBalanceCommitted(
            _vault,
            _user,
            unclaimedReward[_vault][_user],
            _newRewardDebt
    ...
    function claimReward(address _vault, address _user) external {
        ...
        mapping(address => uint256)
            storage vaultUnclaimedReward = unclaimedReward[_vault];
    ...
    function getRewardForBlocksRange(
        ...
        uint256 _startBlock = startBlock;
        if (
            (_fromBlock >= _startBlock && _toBlock >= _fromBlock) &&
            _totalAllocPoint > 0
        ) {
            uint256 _result;
            ...
            unchecked {
                // i >= 1, _toBlock >= _fromBlock
                _epochReward = i > NUMBER_OF_EPOCHS
                    ? 0
                    : epochRewardPerBlock[i - 1];
    ...
    function getPendingReward(
        ...
        if (
            _totalShares > 0 &&
            _vaultInfo.lastRewardBlock != 0 &&
            block.number > _vaultInfo.lastRewardBlock
        ) {
        ...
        return
            IERC20Upgradeable(_vault).balanceOf(_user).mulDiv(
                _rewardPerShare,
                REWARD_PRECISION
            ) +
