 ...
 *  总结：HatsFinance 与 Kleros 法庭的智能合约。允许 HatsFinance 向 Kleros 法庭提交争议并接受裁决
 *       1）初始化：Kleros 法庭合约的地址、额外数据； HatsFinance 的仲裁合约地址、用于争议的元证据、用于计算申请费的乘数。
 *       2）提交争议 => 处理争议 => 申请费和奖励 => 查询和获取信息
 */
