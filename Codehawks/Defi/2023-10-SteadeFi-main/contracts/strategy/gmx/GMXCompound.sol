 ...
 * 总结：GMXCompound 是 Steadefi 杠杆保险库的复利操作的函数和逻辑
 *     - compound 函数从储备（trove）合约中转移 tokenA 和 tokenB 到当前合约，然后获取指定代币 (tokenIn) 的余额。如果余额大于零，它将进行复利操作。
 *     - 检查、取消复利操作
 */
library GMXCompound {
     ...
     * @param self GMXTypes.Store
     */
    function processCompound(GMXTypes.Store storage self) external {
