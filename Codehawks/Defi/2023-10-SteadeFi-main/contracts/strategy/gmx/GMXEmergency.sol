...
library GMXEmergency {
    ...
    event EmergencyClose(uint256 repayTokenAAmt, uint256 repayTokenBAmt);
    ...
    function emergencyPause(GMXTypes.Store storage self) external {
        ...
        GMXManager.removeLiquidity(self, _rlp);
     ...
     * @param self GMXTypes.Store
     */
    function emergencyResume(GMXTypes.Store storage self) external {
     ...
     * @param self GMXTypes.Store
     */
    function processEmergencyResume(GMXTypes.Store storage self) external {
    ...
    function emergencyClose(
        ...
        (_rp.repayTokenAAmt, _rp.repayTokenBAmt) = GMXManager.calcRepay(
            self,
            1e18
        );
    ...
    function emergencyWithdraw(
        ...
        uint256 _userShareBalance = IERC20(address(self.vault)).balanceOf(
            msg.sender
        );
        ...
        uint256 _withdrawAmtTokenA = (_shareRatio *
            self.tokenA.balanceOf(address(this))) / SAFE_MULTIPLIER;
        uint256 _withdrawAmtTokenB = (_shareRatio *
            self.tokenB.balanceOf(address(this))) / SAFE_MULTIPLIER;
