 ...
 * 总结：用于检查 Steadefi 杠杆型保险库相关操作的函数
 *      - 检查存款、提取、撤销、重新平衡等
 */
library GMXChecks {
    /* ====================== CONSTANTS ======================== */
    ...
    function beforeDepositChecks(
        ...
        if (
            !self.vault.isTokenWhitelisted(
                self.depositCache.depositParams.token
            )
        ...
        if (depositValue == 0) revert Errors.InsufficientDepositAmount();
     ...
     * @param self GMXTypes.Store
     */
    function afterDepositChecks(GMXTypes.Store storage self) external view {
  ...
  */
    function beforeWithdrawChecks(GMXTypes.Store storage self) external view {
        if (self.status != GMXTypes.Status.Open)
            revert Errors.NotAllowedInCurrentVaultStatus();
        ...
        if (
            self.withdrawCache.withdrawParams.executionFee <
     ...
     * @param self GMXTypes.Store
     */
    function afterWithdrawChecks(GMXTypes.Store storage self) external view {
        // Guards: revert if lpAmt did not decrease at all
        if (
            GMXReader.lpAmt(self) >= self.withdrawCache.healthParams.lpAmtBefore
    ...
    function beforeRebalanceChecks(
        ...
        if (
            rebalanceType == GMXTypes.RebalanceType.Delta &&
            self.delta == GMXTypes.Delta.Neutral
        ) {
            if (
     ...
     * @param self GMXTypes.Store
     */
    function afterRebalanceChecks(GMXTypes.Store storage self) external view {
     ...
     * @param self GMXTypes.Store
     */
    function beforeCompoundChecks(GMXTypes.Store storage self) external view {
        if (
            self.status != GMXTypes.Status.Open &&
    ...
    function beforeEmergencyWithdrawChecks(
        ...
        if (shareAmt == 0) revert Errors.EmptyWithdrawAmount();
    ...
    function _isWithinStepChange(
        ...
        if (valueBefore == 0) return true;

        return (valueAfter >= (valueBefore * (10000 - threshold)) / 10000 &&
