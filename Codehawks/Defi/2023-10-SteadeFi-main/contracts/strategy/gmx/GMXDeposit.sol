 ...
 * 总结：Steadefi 杠杆保险库的存款操作
 *    - 存款的创建、处理、取消以及失败后的处理流程。
 */
library GMXDeposit {
    ...
    event DepositCreated(address indexed user, address asset, uint256 assetAmt);
    ...
    function deposit(
        ...
        if (self.tokenA.balanceOf(address(this)) > 0) {
            self.tokenA.safeTransfer(
                self.trove,
                self.tokenA.balanceOf(address(this))
            );
        }
        if (self.tokenB.balanceOf(address(this)) > 0) {
            self.tokenB.safeTransfer(
                self.trove,
        ...
        if (isNative) {
            ...
            IERC20(dp.token).safeTransferFrom(
                msg.sender,
                address(this),
                dp.amt
            );
        ...
        (uint256 _borrowTokenAAmt, uint256 _borrowTokenBAmt) = GMXManager
            .calcBorrow(self, _dc.depositValue);
    ...
    function processDepositFailure(
        ...
        if (
            GMXReader.lpAmt(self) <= self.depositCache.healthParams.lpAmtBefore
        ) {
            ...
            _rlp.lpAmt =
                ...
                self.depositCache.healthParams.lpAmtBefore;
            ...
            if (self.delta == GMXTypes.Delta.Long) {
                ...
                (_rlp.minTokenAAmt, _rlp.minTokenBAmt) = GMXManager
    ...
    function processDepositFailureLiquidityWithdrawal(
        ...
        // Repay borrowed assets
        GMXManager.repay(self, _repayTokenAAmt, _repayTokenBAmt);
