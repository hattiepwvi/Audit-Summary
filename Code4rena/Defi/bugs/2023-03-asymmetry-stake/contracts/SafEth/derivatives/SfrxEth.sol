...
contract SfrxEth is IDerivative, Initializable, OwnableUpgradeable {
    ...
    */
    ...
    function withdraw(uint256 _amount) external onlyOwner {
        // 从 SFRX_ETH_ADDRESS 地址中提取 _amount 数量的 safETH 代币，并将其发送到当前合约地址 (address(this))。
        IsFrxEth(SFRX_ETH_ADDRESS).redeem(
            _amount,
            address(this),
            address(this)
        );
        // 获取当前合约地址 (address(this)) 上的 FRX 代币的余额
        uint256 frxEthBalance = IERC20(FRX_ETH_ADDRESS).balanceOf(
            address(this)
        );
        // 最大授权额度设为 frxEthBalance
        ...
        // 将 frxEthBalance 数量的代币进行兑换，其中 1 代表 FRX，0 代表 ETH。
        ...
        // 将当前合约的以太币余额（address(this).balance）发送给消息发送者。
