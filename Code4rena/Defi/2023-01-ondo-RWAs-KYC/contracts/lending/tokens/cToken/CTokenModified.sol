 ...
 * 总结： 用于管理CTokens（Compound Tokens) 的抽象合约，用于处理资金的存入、取出、借入、还款、清算等操作，并提供了一些管理功能和KYC相关的功能。
 *      1） 初始化合约的一些参数，如Comptroller、利率模型、初始兑换率等
 *      2） 函数：
 *             - transferTokens函数用于在账户之间转移代币，并进行一些检查，如检查是否有制裁、转账是否被允许等
 *             - mintInternal和redeemInternal函数用于存入和取出资金。borrowInternal和repayBorrowInternal函数用于借入和还款。liquidateBorrowInternal函数用于清算借款。
 *             - 管理函数：_setPendingAdmin、_acceptAdmin、_setComptroller、_setReserveFactor、_setInterestRateModel等，用于管理合约的管理员、利率模型、储备金因子等。
 *             - KYC（了解您的客户）和安全令牌相关的函数，如setKYCRequirementGroup、setKYCRegistry、_getKYCStatus等。
 *
 */
    ...
    TokenErrorReporter
{
    ...
    function redeemFresh(
        ...
        /* Revert if sanctioned */
        require(
            !sanctionsList.isSanctioned(redeemer),
            "Redeemer is sanctioned"
        );
    ...
    function seizeInternal(
        ...
        require(
            !sanctionsList.isSanctioned(borrower),
    ...
    function setKYCRequirementGroup(uint256 _kycRequirementGroup) external {
        require(
            msg.sender == admin,
            "Only admin can set KYC requirement group"
        );
    ...
    function _setKYCRegistry(address _kycRegistry) internal {
        require(
            ...
            "KYC registry cannot be zero address"
        );
